...
function parseOpenCvMat(opencv_mat) {
  ...
  if (type.includes("CV_32FC")) {
    ...
    switch (dimOpenCv[2]) {
      case 0:
        img = new Uint8Array(opencv_mat.subarray(idx));
        break;
      case 1:
        img = new Int8Array(opencv_mat.subarray(idx));
        break;
      case 2:
        img = new Uint16Array(opencv_mat.subarray(idx));
        break;
      case 3:
        img = new Int16Array(opencv_mat.subarray(idx));
        break;
      case 4:
        img = new Int32Array(opencv_mat.subarray(idx));
        break;
      case 5:
        img = new Float32Array(opencv_mat.subarray(idx));
        break;
      case 6:
        img = new Float64Array(opencv_mat.subarray(idx));
        break;
    }
    numRows = dimOpenCv[0];
    numCols = dimOpenCv[1];
...
export function updateDepthImage(camera, opencv_mat, renderCallback) {
  ...
  promise_img.then((img) => {
    ...
    if (spec.depthImage === null) {
      ...
      for (let i = 0; i < buffer_color.length; i++) {
        buffer_color[i] = 1.0;
      }
